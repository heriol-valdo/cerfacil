...
require_once __DIR__ . '/../../controller/GestionnaireEntreprise/ListGestionnaireEntrepriseController.php';
...
<head>
    <meta charset="UTF-8">
    <title>Liste des Gestionnaires Entreprise | ErpFacil</title>
    ...
    <div class="content-container">
        <header>
            <div class="header-container">
                ...
                <div class="title">
                    <h1>Liste des gestionnaires d'entreprise</h1>
                </div>
            </div>
        </header>
        <main>
            <div class="d-flex justify-content-between mt-3 mb-3">
                ...
                <div>
                    <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addStudentModal">Ajouter un
                        gestionnaire d'entreprise</button>
                </div>
            </div>
                    ...
                    <table class="table table-striped mt-3" id="gestionnaireTable">
                        ...
                        <tbody>
                            <?php foreach ($allgestionnaires as $allgestionnaire): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($allgestionnaire->lastname); ?></td>
                                    <td><?php echo htmlspecialchars($allgestionnaire->firstname); ?></td>
                                    <td><?php echo htmlspecialchars($allgestionnaire->telephone); ?></td>
                                    <td><?php echo htmlspecialchars($allgestionnaire->lieu_travail); ?></td>
                                    <td><?php echo htmlspecialchars($allgestionnaire->nomEntreprise); ?></td>
                                    <td>
                                    <div class="action-buttons">
                                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                                                ...
                                                data-lieu-travail="<?php echo isset($allgestionnaire->lieu_travail) ? htmlspecialchars($allgestionnaire->lieu_travail) : ''; ?>"
                                                data-entreprise="<?php echo isset($allgestionnaire->id_entreprises) ? htmlspecialchars($allgestionnaire->id_entreprises) : ''; ?>">
                                        <i class="fas fa-edit"></i>
                                        </button>
                                        <span class="space"></span>
                                        <?php if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] == 1) : ?>
                                        <button class="btn btn-danger btn-sm" onclick="confirmDeleteGestionnaireEntreprise(<?php echo $allgestionnaire->id_users; ?>)">
            ...
            <?php include 'add-gestionnaire-entreprise.php'; ?>
            <?php include 'edit-gestionnaire-entreprise.php'; ?>
            <?php include 'delete-gestionnaire-entreprise.php'; ?>
            ...
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    $('#id_entreprises').select2({
                        placeholder: "Selectionnez l'entreprise",
                        allowClear: true
                    });
                });
            </script>
