...
$app->delete('/api/deleteCerfa', function (Request $request, Response $response, $param) use ($key) {
    ...
    if ($role === 7 || $role === 3) {
        $cerfa = new Cerfa();
        $facture = new Facture();
        $result = $cerfa->delete($id);
        $resultfacture = $facture->delete($id);
        if (!isset($result['erreur'])) {
            $response->getBody()->write(json_encode(['valid' => "La formation a été supprimée avec succès"]));
