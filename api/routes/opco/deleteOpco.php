...
$app->delete('/api/deleteOpco', function (Request $request, Response $response, $param) use ($key) {
    ...
    if ($role === 7 || $role === 3) {
        $opco = new Opco();
        $result = $opco->delete($id);
        if (!isset($result['erreur'])) {
            $response->getBody()->write(json_encode(['valid' => "L'opco a été supprimée avec succès"]));
