...
require_once __DIR__ . '/../../controller/Entreprise/ListEntrepriseController.php';
...
<head>
    <meta charset="UTF-8">
    <title>Liste des Conseillers Financeurs | ErpFacil</title>
    ...
    <div class="content-container">
        <header>
            <div class="header-container">
                ...
                <div class="title">
                    <h1>Liste des conseillers financeurs</h1>
                </div>
            </div>
        </header>
        <main>
            ...
            <?php if (!empty($allfinanceurs)) : ?>
               
                <div class="table-container">
                    <table class="table table-striped mt-3" id="financeurTable">
                        <thead id="tableHead">
                            <tr>
                                <th>Nom</th>
                                <th>Prénom</th>
                                <th>Type de financeur</th>
                                <th>Entreprise</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($allfinanceurs as $allfinanceur) : ?>
                                <tr>
                                    <td><?= htmlspecialchars($allfinanceur->lastname ?? '') ?></td>
                                    <td><?= htmlspecialchars($allfinanceur->firstname ?? '') ?></td>
                                    <td><?= htmlspecialchars($allfinanceur->type_financeur ?? '') ?></td>
                                    <td><?= htmlspecialchars($allfinanceur->nomEntreprise ?? '') ?></td>
                                    <td>
                      <div class="action-buttons">
                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                                data-userid="<?php echo $allfinanceur->id_users; ?>"
                                data-email="<?php echo $allfinanceur->email; ?>"
                                data-firstname="<?php echo isset($allfinanceur->firstname) ? htmlspecialchars($allfinanceur->firstname) : ''; ?>"
                                data-lastname="<?php echo isset($allfinanceur->lastname) ? htmlspecialchars($allfinanceur->lastname) : ''; ?>"
                                data-typeFinanceur="<?php echo isset($allfinanceur->type_financeur) ? htmlspecialchars($allfinanceur->type_financeur) : ''; ?>"
                                data-entreprise="<?php echo isset($allfinanceur->id_entreprises) ? htmlspecialchars($allfinanceur->id_entreprises) : ''; ?>">
                          <i class="fas fa-edit"></i>
                        </button>
                        <span class="space"></span>
                        <?php if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] == 1) : ?>
                        <button class="btn btn-danger btn-sm" onclick="confirmDeleteFinanceur(<?php echo $allfinanceur->id_users; ?>)">
            ...
            <?php else : ?>
                <p><?= htmlspecialchars($erreur ?? 'Aucun financeur trouvé.') ?></p>
            <?php endif; ?>

            <?php include 'add-financeur.php'; ?>
            <?php include 'edit-financeur.php'; ?>
            <?php include 'delete-financeur.php'; ?>
            ...
            <script>
                function showToast(success, title, message) {
                    ...
                    toastText.innerHTML = `<strong>${title}</strong>: ${message}`;
                ...
                const financeurTable = document.getElementById('financeurTable');
                ...
                function displayPage(page) {
                    const rows = financeurTable.querySelectorAll('tbody tr');
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    function setTableHeight() {
                        const totalRows = financeurTable.querySelectorAll('tbody tr').length;
