...
$allclients = ClientCerfaController::ListAchatClientCerfa();
...
<head>
  <meta charset="UTF-8">
  <title>Liste des  Achats du clients | ErpFacil</title>
  ...
  <div class="content-container">
    <header>
      <div class="header-container">
        ...
        <div class="title">
          <h1>Liste des Achats du client</h1>
        </div>
    ...
    <main>
          ...
          <table class="table table-striped mt-3" id="studentTable">
            ...
            <tbody>
              <?php foreach ($allclients->valid as $allclient) : ?>
                <tr>
                  <td><?php echo htmlspecialchars($allclient->lastname .' '. $allclient->firstname ); ?></td>
                  <td><?php echo htmlspecialchars(date("d/m/Y", strtotime($allclient->date_achat)) ?? ''); ?></td>
                  <td><?php echo htmlspecialchars($allclient->nom ?? ''); ?></td>
                  <td><?php echo htmlspecialchars($allclient->totalFacture ?? ''); ?></td>
                  <td><?php echo htmlspecialchars($allclient->stripe_id ?? ''); ?></td>
                  <td>
                      <div class="action-buttons">
                        <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                                data-totalFacture="<?php echo $allclient->totalFacture; ?>"
                                data-quantite="<?php echo $allclient->quantite; ?>"
                                data-PrixDossier="<?php echo $allclient->prix_dossier; ?>"
                                data-totalDossier="<?php echo $allclient->totalDossier; ?>"
                                data-type="<?php echo $allclient->type; ?>"

                                 data-totalAbonement="<?php echo $allclient->totalAbonement; ?>"
                                 data-PrixAbonement="<?php echo $allclient->prix_abonement; ?>"
                                 data-DateDebut="<?php echo $allclient->date_debut; ?>"
                                 data-DateFin="<?php echo $allclient->date_fin; ?>"
                                 data-StripeID="<?php echo $allclient->stripe_id; ?>"
      ...
      <?php else : ?>
        <p><?php echo $result->erreur ?? 'Aucun Achat client  trouvé.'; ?></p>
      <?php endif; ?>


     
      <?php include 'editDetail-clientCerfa.php'; ?>
