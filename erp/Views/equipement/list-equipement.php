...
require_once __DIR__ . '/../../controller/Equipement/ListEquipementController.php';
...
<head>
  <meta charset="UTF-8">
  <title>Liste des Equipements | ErpFacil</title>
  ...
  <div class="content-container">
    <header>
      <div class="header-container">
        ...
        <div class="title"><h1>Liste des équipements</h1></div>
      </div>
    </header>
    <main>
      <div class="d-flex justify-content-between mt-3 mb-3">
        ...
        <div>
          <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addEquipementModal">Ajouter un
            equipement</button>
        </div>
      </div>
      <?php if (!empty($allequipements)): ?>
        <div class="table-container">
          <table class="table table-striped mt-3" id="equipementsTable">
            <thead id="tableHead">
              <tr>
                <th>Nom</th>
                <th>Quantite</th>
            ...
            <tbody>
              ...
              <?php foreach ($allequipements as $allequipement): ?>
                <tr>
                  <td><?php echo $allequipement->equipement_nom !== null ? htmlspecialchars($allequipement->equipement_nom) : ''; ?></td>
                  <td><?php echo $allequipement->quantite !== null ? htmlspecialchars($allequipement->quantite) : ''; ?></td>
                  <td><?php echo $allequipement->salle_nom !== null ? htmlspecialchars($allequipement->salle_nom) : ''; ?></td>
                  <td>
                    <div class="action-buttons">
                      <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                              data-id="<?php echo $allequipement->id; ?>"
                              data-nom="<?php echo isset($allequipement->equipement_nom) ? htmlspecialchars($allequipement->equipement_nom) : ''; ?>"
                              data-quantite="<?php echo isset($allequipement->quantite) ? htmlspecialchars($allequipement->quantite) : ''; ?>"
                              data-idSalle="<?php echo isset($allequipement->id_salles) ? htmlspecialchars($allequipement->id_salles) : ''; ?>">
                        <i class="fas fa-edit"></i>
                      </button>
                      
                      <span class="space"></span>
                      <button class="btn btn-danger btn-sm" onclick="confirmDeleteEquipement(<?php echo $allequipement->id ?> , <?php echo $allequipement->id_salles ?>)">
      ...
      <?php include 'add-equipement.php'; ?>
      <?php include 'edit-equipement.php'; ?>
      <?php include 'delete-equipement.php'; ?>

      <!-- Modale pour ajouter un équipement -->
            ...
            <div class="modal-body">
              <form id="addForm">
                ...
                <div class="mb-3">
                  ...
                  <input type="number" min="0" name="quantite" id="quantite" class="form-control" min="0" required>
                </div>
                <div class="mb-3">
                  ...
                  <select name="idsalle" id="idsalle" class="form-control" required>
                    ...
                    <?php foreach ($allsalles as $salle) : ?>
                        <option value="<?= htmlspecialchars($salle->id) ?>"><?= htmlspecialchars($salle->nom) ?></option>
                    <?php endforeach; ?>
    ...
    <script>
      ...
      const equipementsTable = document.getElementById('equipementsTable');
      ...
      function displayPage(page) {
        const rows = equipementsTable.querySelectorAll('tbody tr');
      ...
      document.addEventListener("DOMContentLoaded", function() {
        ...
        function setTableHeight() {
          const totalRows = equipementsTable.querySelectorAll('tbody tr').length;
