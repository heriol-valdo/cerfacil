...
document.getElementById('confirmDeleteButton').addEventListener('click', function() {
  if (salleIdToDelete !== null) {
    deleteSalle(salleIdToDelete);
  }
});

async function deleteSalle(id) {
  try {
    const response = await fetch('../../controller/Formation/deleteFormationController.php', {
      method: 'POST',
      headers: {
    ...
    if (result.erreur) {
      showToast(false, result.erreur, "Erreur lors de la suppression de la salle");
