...
require_once __DIR__ . '/../../controller/CentreFormation/listPersonnelByCentre.php';
require_once __DIR__ . '/../../controller/User/validTokenController.php';
$_SESSION['user']['role'] == 1 && require_once __DIR__ . '/../../controller/CentreFormation/ListCentreFormationController.php';
?>
...
<head>
  <meta charset="UTF-8">
  <title>Gestionnaires et formateurs | ErpFacil</title>
  ...
  <div class="content-container">
    <header>
      <div class="header-container">
        ...
        <div class="title">
          <h1>Gestionnaires et formateurs</h1>
        </div>
      </div>
    </header>
    <main>
      <div class="d-flex justify-content-between mt-3 mb-3">
        ...
        <div>
          <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#addFormateurModal">Ajouter un
            formateur</button>
        </div>
      </div>
      <?php if (!empty($list_centre_formateurs)): ?>
        <div class="table-container">
          <table class="table table-striped mt-3" id="formateurTable">
            ...
            <tbody>
              <?php foreach ($list_centre_gestionnaires as $gestionnaire): ?>
                <tr>
                    ...
                    <div class="action-buttons">
                      ...
                      <?php if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] == 1): ?>
                        <button class="btn btn-danger btn-sm"
                          onclick="confirmDeleteFormateur(<?= $gestionnaire->id_users; ?>)">
              ...
              <?php endforeach; ?>
              <?php foreach ($list_centre_formateurs as $allformateur): ?>
                <tr>
                  <td><?php echo $allformateur->lastname !== null ? htmlspecialchars($allformateur->lastname) : ''; ?></td>
                  <td><?php echo $allformateur->firstname !== null ? htmlspecialchars($allformateur->firstname) : ''; ?>
                  </td>
                  <td><?= $allformateur->role ?></td>
                  <td>
                    <div class="action-buttons">
                      <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                        data-userid="<?php echo $allformateur->id_users; ?>"
                        data-email="<?php echo $allformateur->email; ?>"
                        data-firstname="<?php echo isset($allformateur->firstname) ? htmlspecialchars($allformateur->firstname) : ''; ?>"
                        data-lastname="<?php echo isset($allformateur->lastname) ? htmlspecialchars($allformateur->lastname) : ''; ?>"
                        data-telephone="<?php echo isset($allformateur->telephone) ? htmlspecialchars($allformateur->telephone) : ''; ?>"
                        data-adresse="<?php echo isset($allformateur->adressePostale) ? htmlspecialchars($allformateur->adressePostale) : ''; ?>"
                        data-code-postal="<?php echo isset($allformateur->codePostal) ? htmlspecialchars($allformateur->codePostal) : ''; ?>"
                        data-ville="<?php echo isset($allformateur->ville) ? htmlspecialchars($allformateur->ville) : ''; ?>"
                        data-siret="<?php echo isset($allformateur->siret) ? htmlspecialchars($allformateur->siret) : ''; ?>"
                        data-centre="<?php echo isset($allformateur->id_centres_de_formation) ? htmlspecialchars($allformateur->id_centres_de_formation) : ''; ?>">
                        <i class="fas fa-edit"></i>
                      </button>
                      <span class="space"></span>
                      <?php if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] == 1): ?>
                        <button class="btn btn-danger btn-sm"
                          onclick="confirmDeleteFormateur(<?php echo $allformateur->id_users; ?>)">
                          <i class="fas fa-trash-alt"></i>
                        </button>
                      <?php endif; ?>
                    </div>
      ...
      <?php include '../formateur/add-formateur.php'; ?>
      <?php include '../formateur/edit-formateur.php'; ?>
      <?php include '../formateur/delete-formateur.php'; ?>

      <?php include '../gestionnaire-centre/add-gestionnaire-centre.php'; ?>
      <?php include '../gestionnaire-centre/edit-gestionnaire-centre.php'; ?>
      <?php include '../gestionnaire-centre/delete-gestionnaire-centre.php'; ?>
      ...
      <script>
        ...
        const formateurTable = document.getElementById('formateurTable');
        ...
        function displayPage(page) {
          const rows = formateurTable.querySelectorAll('tbody tr');
        ...
        document.addEventListener("DOMContentLoaded", function () {
          ...
          function setTableHeight() {
            const totalRows = formateurTable.querySelectorAll('tbody tr').length;
